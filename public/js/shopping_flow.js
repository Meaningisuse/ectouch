...
function selectShipping(obj)
{
  ...
  {
    document.getElementById('ECT_NEEDINSURE').checked = false;
    document.getElementById('ECT_NEEDINSURE').disabled = true;
  }
  else
  {
    document.getElementById('ECT_NEEDINSURE').checked = false;
    document.getElementById('ECT_NEEDINSURE').disabled = false;
  }
...
function orderShippingSelectedResponse(result)
{
  if (result.need_insure)
  {
    try
    {
      document.getElementById('ECT_NEEDINSURE').checked = true;
    }
    catch (ex)
    {
      alert(ex.message);
  ...
  try
  {
    if (document.getElementById('ECT_CODFEE') != undefined)
    {
      document.getElementById('ECT_CODFEE').innerHTML = result.cod_fee;
    }
  }
  catch (ex)
  {
...
function orderSelectedResponse(result)
{
  ...
  try
  {
    var layer = document.getElementById("ECT_ORDERTOTAL");
...
function changeSurplusResponse(obj)
{
  if (obj.error)
  {
    try
    {
      document.getElementById("ECT_SURPLUS_NOTICE").innerHTML = obj.error;
      document.getElementById('ECT_SURPLUS').value = '0';
      document.getElementById('ECT_SURPLUS').focus();
    }
    catch (ex) { }
  }
  else
  {
    try
    {
      document.getElementById("ECT_SURPLUS_NOTICE").innerHTML = '';
    }
    catch (ex) { }
...
function changeIntegralResponse(obj)
{
  if (obj.error)
  {
    try
    {
      document.getElementById('ECT_INTEGRAL_NOTICE').innerHTML = obj.error;
      document.getElementById('ECT_INTEGRAL').value = '0';
      document.getElementById('ECT_INTEGRAL').focus();
    }
    catch (ex) { }
  }
  else
  {
    try
    {
      document.getElementById('ECT_INTEGRAL_NOTICE').innerHTML = '';
    }
    catch (ex) { }
    orderSelectedResponse(obj.content);
  }
...
function changeBonusResponse(obj)
{
  if (obj.error)
  {
    ...
    try
    {
      document.getElementById('ECT_BONUS').value = '0';
    }
    catch (ex) { }
  }
  else
...
if (obj.error)
  {
    ...
    try
    {
      document.getElementById('ECT_BONUSN').value = '0';
    }
    catch (ex) { }
  }
  else
...
{
  var obj        = document.getElementById('ECT_NEEDINV');
  var objType    = document.getElementById('ECT_INVTYPE');
  var objPayee   = document.getElementById('ECT_INVPAYEE');
  var objContent = document.getElementById('ECT_INVCONTENT');
...
function checkOrderForm(frm)
{
  ...
  // 检查用户输入的余额
  if (document.getElementById("ECT_SURPLUS"))
  {
    var surplus = document.getElementById("ECT_SURPLUS").value;
    ...
    if (error)
    {
      try
      {
        document.getElementById("ECT_SURPLUS_NOTICE").innerHTML = error;
      }
      catch (ex)
      {
      }
  ...
  // 检查用户输入的积分
  if (document.getElementById("ECT_INTEGRAL"))
  {
    var integral = document.getElementById("ECT_INTEGRAL").value;
    ...
    if (error)
    {
      ...
      try
      {
        document.getElementById("ECT_INTEGRAL_NOTICE").innerHTML = error;
      }
      catch (ex)
      {
      }
