...
{
    $editor = '<script id="' . $input_name . '" name="' . $input_name . '" type="text/plain"></script>';
    ...
    $GLOBALS['smarty']->assign('FCKeditor', $editor);
}
...
function get_filter($param_str = '')
{
    ...
    if (isset($_GET['uselastfilter']) && isset($_COOKIE['ECTCP']['lastfilterfile'])
        && $_COOKIE['ECTCP']['lastfilterfile'] == sprintf('%X', crc32($filterfile))) {
        return [
            'filter' => unserialize(urldecode($_COOKIE['ECTCP']['lastfilter'])),
            'sql' => base64_decode($_COOKIE['ECTCP']['lastfiltersql'])
        ];
    } else {
        return false;
    }
