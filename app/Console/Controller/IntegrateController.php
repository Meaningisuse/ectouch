...
class IntegrateController extends InitController
{
    public function index()
    {
        ...
        if ($_REQUEST['act'] == 'list') {
            $modules = read_modules(app_path('plugins/integrate'));
        ...
        if ($_REQUEST['act'] == 'install') {
            ...
            if ($_GET['code'] == 'ectouch') {
                $sql = "UPDATE " . $GLOBALS['ecs']->table('shop_config') . " SET value = 'ectouch' WHERE code = 'integrate_code'";
...
//        }
        ...
        if ($_REQUEST['act'] == 'check_config') {
            ...
            if ($total == 0) {
                ...
                return ecs_header("Location: integrate.php?act=complete\n");
            }
            ...
            $this->smarty->assign('domain', '@ectouch');
        ...
        if ($_REQUEST['act'] == 'import_user') {
            $cfg = session('cfg');
            $ucdb = new Mysql(); // ($cfg['db_host'], $cfg['db_user'], $cfg['db_pass'], $cfg['db_name'], $cfg['db_charset']);TODO
        ...
        if ($_REQUEST['act'] == 'act_modify') {
            ...
            /* 跳转  */
            return ecs_header("Location: integrate.php?act=modify");
        }
        ...
        if ($_REQUEST['act'] == 'task') {
            ...
            $task_del = session('task.del');
            $task_rename = session('task.rename');
            $task_sync = session('task.sync');
            if ($task_del['start'] < $task_del['total']) {
                /* 执行操作 */
                /* 查找要删除用户 */
                $arr = $GLOBALS['db']->getCol("SELECT user_name FROM " . $GLOBALS['ecs']->table('users') . " WHERE flag = 3 LIMIT " . $task_del['start'] . ',' . $result['size']);
                $GLOBALS['db']->query("DELETE FROM " . $GLOBALS['ecs']->table('users') . " WHERE " . db_create_in($arr, 'user_name'));

                /* 保存设置 */
                $result['id'] = 'task_del';
                if ($task_del['start'] + $result['size'] >= $task_del['total']) {
                    session('task.del.start', $task_del['total']);
                    $result['content'] = $GLOBALS['_LANG']['task_complete'];
                } else {
                    session('task.del.start', $task_del['start'] + $result['size']);
                    $result['content'] = sprintf($GLOBALS['_LANG']['task_run'], $task_del['start'], $task_del['total']);
                }

                die(json_encode($result));
            } elseif ($task_rename['start'] < $task_rename['total']) {
                /* 查找要改名用户 */
                $arr = $GLOBALS['db']->getCol("SELECT user_name FROM " . $GLOBALS['ecs']->table('users') . " WHERE flag = 2 LIMIT " . $task_del['start'] . ',' . $result['size']);
                ...
                if ($task_rename['start'] + $result['size'] >= $task_rename['total']) {
                    session('task.rename.start', $task_rename['total']);
                    $result['content'] = $GLOBALS['_LANG']['task_complete'];
                } else {
                    session('task.rename.start', $task_rename['start'] + $result['size']);
                    $result['content'] = sprintf($GLOBALS['_LANG']['task_run'], $task_rename['start'], $task_rename['total']);
                }
                die(json_encode($result));
                ...
                $sql = "SELECT user_name, password, email, sex, birthday, reg_time " .
                    "FROM " . $GLOBALS['ecs']->table('users') . " LIMIT " . $task_del['start'] . ',' . $result['size'];
                ...
                /* 保存设置 */
                $result['id'] = 'task_sync';
                if ($task_sync['start'] + $result['size'] >= $task_sync['total']) {
                    session('task.sync.start', $task_sync['total']);
                    $result['content'] = $GLOBALS['_LANG']['task_complete'];
                } else {
                    session('task.sync.start', $task_sync['start'] + $result['size']);
                    $result['content'] = sprintf($GLOBALS['_LANG']['task_run'], $task_sync['start'], $task_sync['total']);
                }
                die(json_encode($result));
            } else {
                /* 记录合并用户 */
        ...
        if ($_REQUEST['act'] == 'edit_points') {
            ...
            return ecs_header("Location: integrate.php?act=points_set\n");
        }
