...
class OrderController extends InitController
{
    public function index()
    {
        ...
        if ($_REQUEST['act'] == 'list') {
            ...
            $this->smarty->assign('sort_order_time', '<img src="images/sort_desc.gif">');
        ...
        if ($_REQUEST['act'] == 'info') {
            ...
            /* 取得上一个、下一个订单号 */
            if (!empty($_COOKIE['ECTCP']['lastfilter'])) {
                $filter = unserialize(urldecode($_COOKIE['ECTCP']['lastfilter']));
            ...
            $this->smarty->assign('prev_id', $GLOBALS['db']->getOne($sql));
            ...
            $this->smarty->assign('next_id', $GLOBALS['db']->getOne($sql));
            ...
            $this->smarty->assign('agency_list', $GLOBALS['db']->getAll($sql));
            ...
            if ($order['user_id'] > 0) {
                ...
                $this->smarty->assign('address_list', $GLOBALS['db']->getAll($sql));
            }
            ...
            if (isset($_GET['print'])) {
                ...
                $this->smarty->assign('shop_url', $GLOBALS['ecs']->url());
        ...
        GROUP BY DG.goods_id ";
            ...
            if ($order_finish) {
                ...
                if ($cfg == '1') {
                    ...
                    $this->smarty->assign('confirm_url', $GLOBALS['ecs']->url() . 'receive.php?id=' . $order['order_id'] . '&con=' . rawurlencode($order['consignee']));
                    $this->smarty->assign('send_msg_url', $GLOBALS['ecs']->url() . 'user.php?act=message_list&order_id=' . $order['order_id']);
        ...
        if ($_REQUEST['act'] == 'step_post') {
            ...
            if ('user' == $step) {
                ...
                /* 下一步 */
                return ecs_header("Location: order.php?act=" . $step_act . "&order_id=" . $order_id . "&step=goods\n");
            } /* 编辑商品信息 */
            ...
            elseif ('goods' == $step) {
                /* 下一步 */
                if (isset($_POST['next'])) {
                    return ecs_header("Location: order.php?act=" . $step_act . "&order_id=" . $order_id . "&step=consignee\n");
                } /* 完成 */
            ...
            elseif ('consignee' == $step) {
                ...
                if (isset($_POST['next'])) {
                    ...
                    if (exist_real_goods($order_id)) {
                        /* 存在实体商品，去配送方式 */
                        return ecs_header("Location: order.php?act=" . $step_act . "&order_id=" . $order_id . "&step=shipping\n");
                    } else {
                        /* 不存在实体商品，去支付方式 */
                        return ecs_header("Location: order.php?act=" . $step_act . "&order_id=" . $order_id . "&step=payment\n");
                    }
                } elseif (isset($_POST['finish'])) {
                    ...
                    /* 完成 */
                    if ($agency_changed) {
                        return ecs_header("Location: order.php?act=list\n");
                    } else {
                        return ecs_header("Location: order.php?act=info&order_id=" . $order_id . "\n");
                    }
                }
            ...
            elseif ('payment' == $step) {
                ...
                if (isset($_POST['next'])) {
                    /* 下一步 */
                    return ecs_header("Location: order.php?act=" . $step_act . "&order_id=" . $order_id . "&step=other\n");
                } elseif (isset($_POST['finish'])) {
                    /* 初始化提示信息和链接 */
                    $msgs = [];
                    $links = [];
                ...
                if (isset($_POST['next'])) {
                    /* 下一步 */
                    return ecs_header("Location: order.php?act=" . $step_act . "&order_id=" . $order_id . "&step=money\n");
                } elseif (isset($_POST['finish'])) {
                    /* 完成 */
                    return ecs_header("Location: order.php?act=info&order_id=" . $order_id . "\n");
                }
...
//        }
        ...
        if ($_REQUEST['act'] == 'merge') {
            ...
            $this->smarty->assign('order_list', $GLOBALS['db']->getAll($sql));
        ...
        if ($_REQUEST['act'] == 'operate') {
            ...
            if (($require_note && $action_note == '') || isset($show_invoice_no) || isset($show_refund)) {
                ...
                if (!$batch) {
                    /* 一个订单 */
                    return ecs_header("Location: order.php?act=operate_post&order_id=" . $order_id .
                        "&operation=" . $operation . "&action_note=" . urlencode($action_note) . "\n");
                } else {
                    /* 多个订单 */
                    return ecs_header("Location: order.php?act=batch_operate_post&order_id=" . $order_id .
                        "&operation=" . $operation . "&action_note=" . urlencode($action_note) . "\n");
                }
            }
        }
...
//    $action_note = $_REQUEST['action_note'];
    ...
    public function order_list()
    {
        ...
        if ($result === false) {
            ...
            } elseif (isset($_COOKIE['ECTCP']['page_size']) && intval($_COOKIE['ECTCP']['page_size']) > 0) {
                $filter['page_size'] = intval($_COOKIE['ECTCP']['page_size']);
            } else {
                $filter['page_size'] = 15;
            }
