...
class UserMsgController extends InitController
{
    public function index()
    {
        ...
        /*初始化数据交换对象 */
        $exc = new Exchange($GLOBALS['ecs']->table("feedback"), $this->db, 'msg_id', 'msg_title');
        ...
        if ($_REQUEST['act'] == 'insert') {
            ...
            return ecs_header("Location: user_msg.php?act=add&order_id=$_POST[order_id]&user_id=$_POST[user_id]\n");
        }
        ...
        if ($_REQUEST['act'] == 'remove_msg') {
            ...
            return ecs_header("Location: user_msg.php?act=add&order_id=$_GET[order_id]&user_id=$_GET[user_id]\n");
        }
        /*------------------------------------------------------ */
        //-- 更新留言的状态为显示或者禁止
        /*------------------------------------------------------ */
        if ($_REQUEST['act'] == 'check') {
            if ($_REQUEST['check'] == 'allow') {
                ...
                return ecs_header("Location: user_msg.php?act=view&id=$_REQUEST[id]\n");
            } else {
                /* 禁止留言显示 */
        ...
        if ($_REQUEST['act'] == 'list_all') {
            ...
            $this->smarty->assign('sort_msg_id', '<img src="images/sort_desc.gif">');

            $this->smarty->assign('ur_here', $GLOBALS['_LANG']['08_unreply_msg']);
            $this->smarty->assign('full_page', 1);
            return $this->smarty->display('msg_list.htm');
        }
        ...
        if ($_REQUEST['act'] == 'query') {
            ...
            return make_json_result($this->smarty->fetch('msg_list.htm'), '', ['filter' => $msg_list['filter'], 'page_count' => $msg_list['page_count']]);
        }
        /*------------------------------------------------------ */
        //-- ajax 删除留言
        /*------------------------------------------------------ */
        ...
        if ($_REQUEST['act'] == 'action') {
            ...
            if (!empty($_POST['send_email_notice']) or isset($_POST['remail'])) {
                ...
                $this->smarty->assign('shop_name', "<a href='" . $GLOBALS['ecs']->url() . "'>" . $GLOBALS['_CFG']['shop_name'] . '</a>');
            ...
            return ecs_header("Location: ?act=view&id=" . $_REQUEST['msg_id'] . "&send_ok=$send_ok\n");
        }
        ...
        if ($_REQUEST['act'] == 'drop_file') {
            ...
            return ecs_header("Location: user_msg.php?act=view&amp;id=" . $_GET['id'] . "\n");
        }
    }
    ...
    protected function get_feedback_detail($id)
    {
        $sql = "SELECT T1.*, T2.msg_id AS reply_id, T2.user_name  AS reply_name, u.email AS reply_email, " .
            ...
            "LEFT JOIN " . $GLOBALS['ecs']->table('admin_user') . " AS u ON u.user_id='" . session('admin_id') . "' " .
