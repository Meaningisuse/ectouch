...
class ArticleController extends InitController
{
    ...
    {
        /*初始化数据交换对象 */
        $exc = new Exchange($GLOBALS['ecs']->table("article"), $this->db, 'article_id', 'title');
        //$image = new Image();
        ...
        if ($_REQUEST['act'] == 'add_link_goods') {
            ...
            $add_ids = json_decode($_GET['add_ids'], true);
            $args = json_decode($_GET['JSON'], true);
            $article_id = $args[0];
    ...
    /* 把商品删除关联 */
    protected function drop_link_goods($goods_id, $article_id)
    {
        $sql = "DELETE FROM " . $GLOBALS['ecs']->table('goods_article') .
    ...
    /* 取得文章关联商品 */
    protected function get_article_goods($article_id)
    {
        $list = [];
    ...
    /* 获得文章列表 */
    protected function get_articleslist()
    {
        $result = get_filter();
        if ($result === false) {
            $filter = [];
    ...
    /* 上传文件 */
    protected function upload_article_file($upload)
    {
