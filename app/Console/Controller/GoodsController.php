...
class GoodsController extends InitController
{
    public function index()
    {
        ...
        if ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy') {
            ...
            if ($is_add) {
                /* 默认值 */
                $last_choose = [0, 0];
                if (!empty($_COOKIE['ECTCP']['last_choose'])) {
                    $last_choose = explode('|', $_COOKIE['ECTCP']['last_choose']);
                }
                $goods = [
                    'goods_id' => 0,
                    'goods_desc' => '',
        ...
        if ($_REQUEST['act'] == 'get_goods_list') {
            $filters = json_decode($_GET['JSON'], true);
        ...
        if ($_REQUEST['act'] == 'add_link_goods') {
            ...
            $linked_array = json_decode($_GET['add_ids'], true);
            $linked_goods = json_decode($_GET['JSON'], true);
        ...
        if ($_REQUEST['act'] == 'drop_link_goods') {
            ...
            $drop_goods = json_decode($_GET['drop_ids'], true);
        ...
        if ($_REQUEST['act'] == 'add_group_goods') {
            ...
            $fittings = json_decode($_GET['add_ids'], true);
        ...
        if ($_REQUEST['act'] == 'drop_group_goods') {
            ...
            $fittings = json_decode($_GET['drop_ids'], true);
            $arguments = json_decode($_GET['JSON'], true);
            $goods_id = $arguments[0];
            $price = $arguments[1];
        ...
        if ($_REQUEST['act'] == 'get_article_list') {
            $filters = (array)json_decode(json_str_iconv($_GET['JSON'], true));
        ...
        if ($_REQUEST['act'] == 'add_goods_article') {
            ...
            $articles = json_decode($_GET['add_ids'], true);
            $arguments = json_decode($_GET['JSON'], true);
            $goods_id = $arguments[0];
        ...
        if ($_REQUEST['act'] == 'drop_goods_article') {
            ...
            $articles = json_decode($_GET['drop_ids'], true);
            $arguments = json_decode($_GET['JSON'], true);
            $goods_id = $arguments[0];
     ...
     * @return boolean
     */
    protected function goods_parse_url($url)
    {
     ...
     * @return  void
     */
    protected function handle_volume_price($goods_id, $number_list, $price_list)
    {
     ...
     * @return  bool
     */
    protected function update_goods_stock($goods_id, $value)
    {
        if ($goods_id) {
