...
class PrivilegeController extends InitController
{
    public function index()
    {
        ...
        /* 初始化 $exc 对象 */
        $exc = new Exchange($GLOBALS['ecs']->table("admin_user"), $this->db, 'user_id', 'user_name');
        ...
        if ($_REQUEST['act'] == 'signin') {
            if (intval($GLOBALS['_CFG']['captcha']) & CAPTCHA_ADMIN) {
                /* 检查验证码是否正确 */
                if ($this->checkVerify(input('captcha'))) {
        ...
        if ($_REQUEST['act'] == 'remove') {
            ...
            if ($exc->drop($id)) {
                // $sess->delete_spec_admin_session($id); // 删除session中该管理员的记录，强制下线TODO
