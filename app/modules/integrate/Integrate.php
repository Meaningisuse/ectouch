...
class Integrate
{
    ...
    public function remove_user($id)
    {
        ...
        if ($this->need_sync || (isset($this->is_ectouch) && $this->is_ectouch)) {
            /* 如果需要同步或是ectouch插件执行这部分代码 */
        ...
        if (isset($this->ectouch) && $this->ectouch) {
            /* 如果是ectouch插件直接退出 */
            return;
        }
    ...
    public function set_cookie($username = '', $remember = null)
    {
        if (empty($username)) {
            /* 摧毁cookie */
            $time = time() - 3600;
            setcookie("ECT[user_id]", '', $time, $this->cookie_path);
            setcookie("ECT[password]", '', $time, $this->cookie_path);
        } elseif ($remember) {
            /* 设置cookie */
            $time = time() + 3600 * 24 * 15;

            setcookie("ECT[username]", $username, $time, $this->cookie_path, $this->cookie_domain);
            $sql = "SELECT user_id, password FROM " . $GLOBALS['ecs']->table('users') . " WHERE user_name='$username' LIMIT 1";
            $row = $GLOBALS['db']->getRow($sql);
            if ($row) {
                setcookie("ECT[user_id]", $row['user_id'], $time, $this->cookie_path, $this->cookie_domain);
                setcookie("ECT[password]", $row['password'], $time, $this->cookie_path, $this->cookie_domain);
            }
        }
    }
