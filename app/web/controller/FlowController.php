...
class FlowController extends InitController
{
    public function index()
    {
        ...
        if ($_REQUEST['step'] == 'add_to_cart') {
            ...
            return ecs_header("Location:./flow.php\n");
            ...
            if ($_SERVER['REQUEST_METHOD'] == 'GET') {
                ...
                if (!empty($_POST['act']) && $_POST['act'] == 'signin') {
                    ...
                    if ($this->user->login($_POST['username'], $_POST['password'], isset($_POST['remember']))) {
                        ...
                        if ($GLOBALS['db']->getOne($sql) > 0) {
                            return ecs_header("Location: flow.php?step=checkout\n");
                        } else {
            ...
            if ($_SERVER['REQUEST_METHOD'] == 'GET') {
                /* 取得购物类型 */
                $flow_type = session('?flow_type') ? intval(session('flow_type')) : CART_GENERAL_GOODS;
                ...
                if (session('user_id') > 0) {
                    ...
                    if (count($consignee_list) < 5) {
                        ...
                        $consignee_list[] = ['country' => $GLOBALS['_CFG']['shop_country'], 'email' => session('?email') ? session('email') : ''];
                    }
                } else {
            ...
            if (empty(session('direct_shopping')) && session('user_id') == 0) {
                ...
                return ecs_header("Location: flow.php?step=login\n");
            }
            ...
            if (isset($payment_list)) {
                foreach ($payment_list as $key => $payment) {
                    if ($payment['is_cod'] == '1') {
                        $payment_list[$key]['format_pay_fee'] = '<span id="ECT_CODFEE">' . $payment['format_pay_fee'] . '</span>';
                    }
                    /* 如果有余额支付 */
        ...
        elseif ($_REQUEST['step'] == 'done') {
            ...
            $order['from_ad'] = !empty(session('from_ad')) ? session('from_ad') : '0';
...
//    die(sprintf($GLOBALS['_LANG']['bonus_is_ok'], price_format($bonus['type_money'], false)));
        ...
        elseif ($_REQUEST['step'] == 'add_package_to_cart') {
            ...
            $package = json_decode($_POST['package_info'], true);
