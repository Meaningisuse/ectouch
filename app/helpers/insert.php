...
function insert_history()
{
    ...
    if (!empty($_COOKIE['ECT']['history'])) {
        $where = db_create_in($_COOKIE['ECT']['history'], 'goods_id');
...
function insert_member_info()
{
    ...
    if (session('user_id') > 0) {
        ...
        if (!empty($_COOKIE['ECT']['username'])) {
            $GLOBALS['smarty']->assign('ecs_username', stripslashes($_COOKIE['ECT']['username']));
        }
