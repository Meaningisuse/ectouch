...
{
    throw new App\Kernel\Exception($msg, $code);
}
 ...
 * @return void|array
 */
function trace($value = '[ectouch]', $label = '', $level = 'DEBUG', $record = false)
{
    return App\Kernel\Kernel::trace($value, $label, $level, $record);
}
...
function I($name, $default = '', $filter = null, $datas = null)
{
    ...
    is_array($data) && array_walk_recursive($data, '_filter');
    return $data;
}
...
function D($name = '', $layer = '')
{
    ...
    if (class_exists($class)) {
        ...
        // 自动加载公共模块下面的模型
        $class = 'App\\Common\\' . $layer . '\\' . $name . $layer;
        $model = class_exists($class) ? new $class($name) : new App\Kernel\Model($name);
    } else {
        App\Kernel\Log::record('D方法实例化没找到模型类' . $class, App\Kernel\Log::NOTICE);
        $model = new App\Kernel\Model(basename($name));
    }
...
{
    \App\Kernel\Hook::listen($tag, $params);
}
...
function B($name, $tag = '', &$params = null)
{
    ...
    return \App\Kernel\Hook::exec($name, $tag, $params);
}
 ...
 * 自定义异常处理
 * @param string $msg 异常消息
 * @param string $type 异常类型 默认为App\Kernel\Exception
 * @param integer $code 异常代码 默认为0
 * @return void
 */
function throw_exception($msg, $type = 'App\\Kernel\\Exception', $code = 0)
{
    App\Kernel\Log::record('建议使用E方法替代throw_exception', App\Kernel\Log::NOTICE);
    if (class_exists($type, false)) {
        throw new $type($msg, $code);
    } else {
        App\Kernel\Kernel::halt($msg);
    }
    // 异常类型不存在则输出错误信息字串
}
...
function U($url = '', $vars = '', $suffix = true, $domain = false)
{
    ...
    if (0 == C('URL_MODEL')) {
        ...
        if (isset($route)) {
            ...
            if (C('URL_ROUTER_ON')) {
                $url = App\Kernel\Route::reverse($path, $vars, $depr, $suffix);
                if (!$url) {
                    $url = $path;
                }
            } else {
...
function S($name, $value = '', $options = null)
{
    ...
    if (is_array($options)) {
        ...
        $cache = App\Kernel\Cache::getInstance($type, $options);
    } elseif (is_array($name)) {
        // 缓存初始化
        $type  = isset($name['type']) ? $name['type'] : '';
        $cache = App\Kernel\Cache::getInstance($type, $name);
        return $cache;
    } elseif (empty($cache)) {
        // 自动初始化
        $cache = App\Kernel\Cache::getInstance();
    }
    if ('' === $value) {
        // 获取缓存
...
function F($name, $value = '', $path = DATA_PATH)
{
    ...
    if ('' !== $value) {
        if (is_null($value)) {
            ...
            if (false !== strpos($name, '*')) {
                ...
                unset($_cache[$name]);
                return App\Kernel\Storage::unlink($filename, 'F');
            }
        } else {
            App\Kernel\Storage::put($filename, serialize($value), 'F');
            // 缓存数据
            $_cache[$name] = $value;
            return null;
        }
    ...
    if (App\Kernel\Storage::has($filename, 'F')) {
        $value         = unserialize(App\Kernel\Storage::read($filename, 'F'));
        $_cache[$name] = $value;
    } else {
        $value = false;
    }
...
function session($name = '', $value = '')
{
    ...
    if (is_array($name)) {
        ...
        if (C('SESSION_TYPE')) {
            // 读取session驱动
            $type   = C('SESSION_TYPE');
            $class  = strpos($type, '\\') ? $type : 'App\\Kernel\\Session\\Driver\\' . ucwords(strtolower($type));
            $hander = new $class();
