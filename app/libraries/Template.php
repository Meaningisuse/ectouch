...
class Template
{
    ...
    {
        if (!defined('ECT_ADMIN')) {
            $source = $this->smarty_prefilter_preCompile($source);
        } else {
            $tmp_dir = asset('static/admin') . '/';
            $pattern = [
                '/(href=["|\'])(styles|vendor)(\/.*?)(["|\'])/i',  // 替换相对链接
                '/((?:background|src)\s*=\s*["|\'])(?:\.\/|\.\.\/)?(styles|vendor)(\/.*?["|\'])/is', // 在images前加上 $tmp_dir
                '/((?:background|background-image):\s*?url\()(?:\.\/|\.\.\/)?(images|vendor)(\/)/is', // 在images前加上 $tmp_dir
            ];
            $replace = [
                '\1' . $tmp_dir . '\2\3\4',
                '\1' . $tmp_dir . '\2\3',
        ...
        foreach ($arr as $val) {
            if (in_array($val, $scripts) == false) {
                ...
                if ($val{0} == '.') {
                    ...
                    $str .= '<script type="text/javascript" src="' . asset((defined('ECT_ADMIN') ? 'static/admin/' : '') . 'js/' . $val) . '"></script>' . "\n";
                }
            }
        }
